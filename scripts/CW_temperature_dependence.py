...
for T in np.arange(T_l, T_r, T_step):
    ...
    for count in range(0, 10000):
        ...
        if result.fun < .000001:
            T_.append(T)
            M.append(result.x[0])
            if susceptibility(result.x[0])>0:
                J_stable.append(J)
                M_stable.append(result.x[0])

axs[0].plot(T_, M, '*', markersize=1, color='orange', label='Theor', zorder=100)
# axs[0].plot(J_stable, M_stable, '*', markersize=1)
...
axs[0].plot(T_sim[0], M_avrg[0], '*', label='Sim', markersize=3, color='blue', alpha=.5)
axs[0].plot(T_sim[1], M_avrg[1], '*', markersize=1, color='blue', alpha=.5)
...
axs[1].plot(T_sim[0], M_avrg[0], '*', label='Sim', markersize=3, color='blue', alpha=.5)
axs[1].plot(T_sim[1], M_avrg[1], '*', markersize=1, color='blue', alpha=.5)
