...
public:
  ...
  matrix(const unsigned int&, const unsigned int&, const bool init_0=false);
  matrix(const unsigned int&, const bool init_0=false); //Create square matrix
  ...
  void clear();
  matrix<T> &set_all_entries_to(const T&);
