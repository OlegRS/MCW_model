...
template <typename T> matrix<T>::matrix(const unsigned int &dim_y_, const unsigned int &dim_x_, const bool init_0)
...
template <typename T> matrix<T>::matrix(const unsigned int &dim_x_, const bool init_0) : matrix(dim_x_, dim_x_, init_0) {}
...
template <typename T> matrix<T>& matrix<T>::set_all_entries_to(const T &value) {
  unsigned int sz = dim_x*dim_y;
  for(unsigned int i=0; i<sz; ++i)
    array[i] = value;
  return *this;
